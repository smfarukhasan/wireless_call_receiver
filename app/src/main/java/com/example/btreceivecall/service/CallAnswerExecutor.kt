...
class CallAnswerExecutor(private val service: AccessibilityService) {
    ...
    fun clickAnswerNode(): Boolean {
        ...
        for (root in roots) {
            ...
            if (answerNode != null) {
                Log.i(TAG, "Found target Answer node: pkg=${answerNode.packageName}, id=${answerNode.viewIdResourceName}, text='${answerNode.text}'")
    ...
    private fun getAllActiveRoots(): List<AccessibilityNodeInfo> {
        ...
        service.rootInActiveWindow?.let {
            if (it.packageName?.toString() != CallAppRegistry.OWN_PACKAGE) roots.add(it)
        }
        try {
            service.windows?.forEach { window ->
                window.root?.let { root ->
                    if (root.packageName?.toString() != CallAppRegistry.OWN_PACKAGE && roots.none { it == root }) {
                        roots.add(root)
                    }
                }
            }
