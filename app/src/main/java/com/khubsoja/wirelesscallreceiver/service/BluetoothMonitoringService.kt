...
import com.khubsoja.wirelesscallreceiver.ui.MainActivity
...
class BluetoothMonitoringService : Service() {
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "wireless_call_receiver_channel"
        ...
        fun startService(context: Context) {
            if (isServiceRunning && instance != null) return
    ...
    private fun acquireCallFocus() {
        try {
            ...
            // STEP 1: Request Audio Focus FIRST (this changes STREAM_MUSIC volumes)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // STEP 2: Lock MediaSession to capturing
            mediaSessionController.setCapturing(true)

            // STEP 3: Start silent audio loop to anchor AVRCP routing (this also changes STREAM_MUSIC)
            startSilentAudioLoop()
            Log.i(TAG, "Bluetooth headset buttons locked to MediaSession.")

            // STEP 4: Register volume observer AFTER audio setup with delay.
            // Android's audio routing changes STREAM_MUSIC volumes during requestAudioFocus()
            // and startSilentAudioLoop(). If we register the monitor before this, those
            // system-level changes are misinterpreted as user button presses causing false
            // auto-answer. We delay registration by 1.5s to let all system audio settle first.
            mainHandler.postDelayed({
                if (!volumeMonitor.isRegistered) {
                    volumeMonitor.register()
                    Log.d(TAG, "Volume monitor registered after audio settle delay.")
                }
            }, 700L)
        } catch (e: Exception) {
